...
const permitAdmin = (req, res, next) => {
    try {
        ...
        if (res.locals.decoded.type < 3) {
            return res.status(400).json({ error: { status: 1, message: "Insufficient privileges (Admin API)" }, data: {}, message: {} });
        }
...
const permitManager = (req, res, next) => {
    try {
        ...
        if (res.locals.decoded.type < 2) {
            return res.status(400).json({ error: { status: 1, message: "Insufficient privileges (Manager API)" }, data: {}, message: {} });
        }
